...
state_info states[2]; //curr-state, next-state 으로 추정
...
int main(int argc, char *argv[]) {
	...
	field_info fields;
	retirement_info retired = {0, NOP_INSTR}; //아직 retired된 inst가없다.
	FILE *in_file;
...
#ifdef HYU_ITE2031
	if (PCWrite) {
		// TODO: Determine branch and change the control flow
		inst=curr->IFID.Inst;
		...
		curr->pc = next->pc;
		if(ctl.Branch){
			int vala=forwardIDA(curr,curr->regs->regs[fields.rs]);
			int valb=forwardIDB(curr,curr->regs->regs[fields.rt]);
			if(vala==valb){
				branch_taken=1;
			}
			if(branch_taken){
			curr->pc=curr->IFID.PC+(fields.imm<<2);
			}
		}
		if(ctl.Jump){
			jump_taken=1;
			// if j
			if(get_inst_type(inst)==J){
				curr->pc=(curr->IFID.PC & 0xf0000000) | (fields.addr << 2);
			}else if(get_inst_type(inst)==JAL){
				reg_file_write(31,curr->IFID.PC,NULL);
				next->IDEX.RegWrite=DEASSERT;
				curr->pc=(curr->IFID.PC & 0xf0000000) | (fields.addr <<2);
			}else if(get_inst_type(inst)==JR){
				curr->pc=forwardIDA(curr,curr->regs->regs[fields.rs]);
			}
			// else if jal
			// else if jr 
		} 
